...
export default function Relatorios() {
    ...
    (async () => {
      try {
        const [r, t, p] = await Promise.all([
          ...
          api.get("/relatorios/top-produtos?limite=5"),
          api.get("/relatorios/pagamentos"),
        ]);
  ...
  return (
    <>
      <div className="page-header"><h2>📈 Relatórios & Dashboard</h2></div>
      ...
      <div className="card">
        ...
        {top.length === 0 ? <p style={{ opacity: 0.6 }}>Nenhuma venda registrada ainda.</p> : top.map((p) => (
          <div key={p.id} className="bar-row">
            ...
            <div className="bar-track"><div className="bar-fill" style={{ width: `${(Number(p.qtd) / maxQtd) * 100}%` }} /></div>
