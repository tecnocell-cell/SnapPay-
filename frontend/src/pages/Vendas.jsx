...
export default function Vendas({ apiUrl }) {
  ...
  async function verDetalhe(id) {
    const res = await fetch(`${apiUrl}/vendas/${id}`);
    setDetalhe(await res.json());
  }

  async function cancelar(id) {
    if (!confirm(`Cancelar a venda #${id}? O estoque será devolvido.`)) return;
    const res = await fetch(`${apiUrl}/vendas/${id}/cancelar`, { method: "POST" });
    if (res.ok) { setDetalhe(null); carregar(); }
    ...
    <>
      ...
      <div className="card">
        <table className="data-table">
          <thead>
            <tr><th>#</th><th>Data</th><th>Cliente</th><th>Pagamento</th><th>Total</th><th>Status</th><th></th></tr>
          </thead>
          <tbody>
            {vendas.length === 0 ? (
              ...
              <tr key={v.id}>
                <td>#{v.id}</td>
                <td>{dataBR(v.finalizada_em || v.aberta_em)}</td>
      ...
      {detalhe && (
        <div className="modal-overlay" onClick={() => setDetalhe(null)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            ...
            <table className="data-table">
              ...
              <tbody>
                {detalhe.itens.map((it) => (
                  <tr key={it.id}>
                    <td>{it.nome.substring(0, 30)}</td>
                    <td>{Number(it.quantidade)}</td>
            ...
            <div className="linha total" style={{ marginTop: 12 }}>
